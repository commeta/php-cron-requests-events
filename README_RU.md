...
- Умеет исполнять отложенные функции в фоновом режиме.
...
При подборе параметра `$cron_settings['delay']` можно посмотреть в логи сервера, обычно хост ежеминутно опрашивается массой ботов.
...
Для отложенного запуска функции в фоновом режиме, достаточно добавить в список задач функцию с заданным интервалом `interval=> 0`
...
- Возможна передача параметров через сессионный файл
