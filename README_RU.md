...
Для отложенного запуска функции в фоновом режиме, достаточно добавить в список задач функцию с заданным интервалом или временем запуска.
Укажите режим запуска `multithreading=> true` и подключите include('cron.php') в вашем скрипте.
