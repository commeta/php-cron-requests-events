...
- Позволяет [исполнять функции в параллельном процессе](#параллельный-запуск-функций).
- Использует [Алгоритм реализации транзакции на PHP с помощью блокировки файла](https://github.com/commeta/fast_io/blob/main/test/transaction/README.md)
...
- Минимальный объем PHP кода:  38.2 Kб (23.7 Kб без пробелов и комментариев)
...
$cron_requests_events_settings= [
	'log_file'=> $cron_requests_events_root . 'cron.log', // Путь к файлу журнала, пустая строка '' - отключает журнал
...
Выражение CRON - это строка, представляющая расписание выполнения конкретной команды. Параметры расписания CRON:

    *    *    *    *    *
    -    -    -    -    -
    |    |    |    |    |
    |    |    |    |    |
    |    |    |    |    +----- день недели (0 - 7) (воскресенье=0)
    |    |    |    +---------- месяц (1 - 12)
    |    |    +--------------- день месяца (1 - 31)
    |    +-------------------- час (0 - 23)
    +------------------------- мин (0 - 59)
...
Каждая из частей поддерживает подстановочные знаки `(*)`, диапазоны `(2-5)`, интервалы `(*/2)` и списки `(2,5,6,11)`

- `'crontab'` - Базовый синтаксис cron
...
- `$cron_requests_events_start`, запрещает автоматический запуск управляющего процесса


Переменные сохраняют свои значения между запусками задач по расписанию, но до тех пор пока поля в `$cron_requests_events_jobs[$job_process_id]` не будут изменены.
```
...
- Укажите флаг запуска `$cron_requests_events_start= false;`
...
Функция обработчик `get_param()` определенная в файле `cron.php` будет запущена немедленно, в параллельном процессе.
...
- все кадры располагаются в файле со смещением
