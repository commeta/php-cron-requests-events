...
За распределением прерываний между ядрами процессора следит Linux Kernel и Apache\Nginx worker, по умолчанию операционная система сама распределяет CPU bound нагрузку между свободными ядрами, в данном случае ей еще помогает управляющий worker веб сервера.
