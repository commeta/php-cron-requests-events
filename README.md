...
- Подходит для SHARED хостинга (возможны ограничения), не использует системный CRON.
- Пригодится когда есть только FTP доступ к серверу или загрузка файлов через админку.
- Позволяет в резидентном режиме выполнять по расписанию PHP функции или include callback скрипты, с интервалом от секунды.
