...
- declare(strict_types=1); // Строгая типизация для PHP > 7.0, поддерживает opcache.jit компилятор
- $host= "localhost"; // Для запуска через консоль CLI, впишите имя вашего домена
...
OPCache memory consumption: 69.48KB (PHP 8.2 данные, строки, байткод, DynASM, служебка). Jit buffer: 18.0KB
