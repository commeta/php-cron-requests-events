...
#### Функции
- queue_push($multicore_long_time_micro_job); // поместить в очередь
1. блокирующая операция
2. ждет освобождения файла очереди
...
- $multicore_long_time_micro_job= queue_pop(); // забрать из очереди
1. блокирующая операция
2. ждет освобождения файла очереди
