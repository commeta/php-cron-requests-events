...
В обоих функциях применяются низкоуровневые операции, оптимизатор кода и jit компилятор сокращают задержки. Данные читаются и записываются кадрами, кадры могут быть произвольных размеров. Основная работа с файлом происходит в последних секторах, благодаря этому данные легко буферизируются несколькими слоями Zend Engine и ядра операционной системы.
