...
```
В зависимости от величины списка задач, характера задач, и пр. можно регулировать задержку в пределах 1 до 60 секунд.
...
- при отключенных журналах в основном потоке во время простоя, нагрузка будет только по write(2) syscalls for the process, раз в секунду 1 вызов на задачу, только функции времени
