...
В резидентном режиме список задач будет проверятся в непрерывном цикле, по умолчанию установлено значение:
```
sleep(1); // delay, min values increases load on server: read(2) syscalls for the process
```
В зависимости от величины списка задач, характера задач, требуемой задержки, и пр. можно регулировать задержку в пределах 1 до 60 секунд.
...
6. Резидентный режим: 380Кб оперативной памяти, приоритет 35 (nice 15)
- при значении CRON_DELAY = 0 управляющий процесс будет запущен постоянно. 
- при отключенных журналах в основном потоке во время простоя, нагрузка будет только по read(2) syscalls for the process, раз в секунду 1 вызов на задачу, только функции времени
- во время исполнения задачи тоже происходят syscalls, кэшируемые системные вызовы include, shell_exec 
