...
За распределением прерываний между ядрами процессора следит Linux Kernel, по умолчанию операционная система сама распределяет CPU bound нагрузку между свободными ядрами.
